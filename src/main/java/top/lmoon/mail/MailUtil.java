...
//	private static String receiveMailAccount = "guozy@staff.tianya.cn";
	...
	private static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail,
			String sendUserName, String receiveUserName, String title, String content) throws Exception {
		// 1. 创建一封邮件
		MimeMessage message = new MimeMessage(session);

		// 2. From: 发件人（昵称有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改昵称）
		// message.setFrom(new InternetAddress(sendMail, "某东网", "UTF-8"));
		message.setFrom(new InternetAddress(sendMail, sendUserName, "UTF-8"));

		// 3. To: 收件人（可以增加多个收件人、抄送、密送）
		// message.setRecipient(MimeMessage.RecipientType.TO, new
		// InternetAddress(receiveMail, "XX用户", "UTF-8"));
		...
		// message.setContent("XX用户_你好, 今天$全%场5折, 快R来%抢#购,^ 错*过今天再等一年。。。",
